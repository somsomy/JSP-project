...
for(int i = 0; i <cbList.size(); i++) {
	...
	%>
	<td><table onclick="location.href='catsContent.jsp?catId=<%=cb.getCatId()%>'" class="ctable">
...
<a href="cats.jsp?pageNum=<%=startPage-pageBlock%>">이전</a>
<%} 
for(int i=startPage;i<=endPage;i++){
	%>
	<a href="cats.jsp?pageNum=<%=i%>"><%=i %></a>
	<%
}
%>
<%
if(endPage < pageCount){
	%>
	<a href="cats.jsp?pageNum=<%=startPage+pageBlock%>">다음</a>
	<%
}
%>
</div>
